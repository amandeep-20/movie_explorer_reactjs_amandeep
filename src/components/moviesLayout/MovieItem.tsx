...
import { Box, Typography, IconButton, Tooltip, Chip, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, Button } from '@mui/material';
...
const MovieItem: React.FC<MovieItemProps> = ({ episode, index, role, subscriptionPlan = 'none', onDelete }) => {
  ...
  const handleDeleteClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log(`Initiating delete for movie: ${episode.title} (ID: ${episode.id})`);
    if (isSupervisor) {
      setIsModalOpen(true);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    console.log(`Confirmed delete for movie: ${episode.title} (ID: ${episode.id})`);
